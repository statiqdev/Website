...
This will result in 4 documents, one for each of the apples. It uses the shorthand [`Meta`](xref:web-shortcodes#meta) shortcode syntax to output the name of the current enumerated value, but could also have used [Razor](xref:web-razor) syntax or another templating format.
...
We need to change the `DestinationFileName` setting in this case since the [computed value](xref:documents-and-metadata#about-metadata#computed-values) will need to compile when a `Current` value isn't available (so it must be accessed through the `Document`). This example also uses [Razor](xref:web-razor) syntax (and would presumably be named `apple.cshtml`) given the complexity of the logic. The [`If`](xref:web-shortcodes#if) and [`ForEach`](xref:web-shortcodes#foreach) shortcodes _could_ actually handle this in a [Markdown](xref:web-markdown) file as well, but the syntax wouldn't be as easy to read for the purpose of this example.
