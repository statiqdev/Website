Events let you hook into various stages of the generation process and can be helpful when you need to implement cross-cutting behavior at runtime or when you need to modify the behavior of existing [pipelines](xref:pipelines). Statiq has a global event mechanism that makes it easy to subscribe to and handle events.
